...
// ─── Main Carte ───────────────────────────────────────────────────────────────
const Carte = ({ targetLieuId = null, onTargetConsumed = () => {} }) => {
        ...
        onEachFeature: (feat, l) => {
          ...
          l.bindPopup(`
            <div style="min-width:200px">
              <h3 style="margin:0 0 8px;color:${couleur}">${bourg.nom}</h3>
              <p style="margin:4px 0"><strong>Clan :</strong> ${clan?.nom || 'Inconnu'}</p>
              ${bourg.description?.richesse ? `<p style="margin:4px 0"><strong>Richesse :</strong> ${'★'.repeat(bourg.description.richesse)}</p>` : ''}
              ${bourg.description?.ambiance ? `<p style="margin:4px 0"><strong>Ambiance :</strong> ${bourg.description.ambiance}</p>` : ''}
            </div>`, { maxWidth: 300 });
        },
      });
      layer.addTo(map);
  ...
  // ── Consume targetLieuId from LieuxTable navigation ──────────────────────
  useEffect(() => {
  ...
  const handleLieuClick = (lieu) => {
    ...
    mapInstanceRef.current.flyTo([lieu.latitude, lieu.longitude], 15, { duration: 0.8 });
    // Close popup of previous, open for new marker
    setTimeout(() => {
